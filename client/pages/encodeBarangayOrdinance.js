...
function encodeBarangayOrdinance() {
    ...
    const submitDocument = async () => {
        if (file && dropdownMenuValueBarangay != "Barangay") {
            ...
            await Axios.post(
                "http://localhost:3001/barangayOrdinance/getBarangayOrdinanceYear",
                data
            ).then(async (res) => {
                if (!res.data) {
                    setLoading(true);
                    ...
                    if (extension == "doc" || extension == "docx") {
                        const documentName = `BarangayOrdinance${response.selectedBarangay}${response.selectedDistrict}${yearSubmitted}.pdf`;
                        ...
                        const documentName = `BarangayOrdinance${response.selectedBarangay}${response.selectedDistrict}${yearSubmitted}.${extension}`;
    ...
    return (
        <div className="flex flex-col w-full max-w-5xl">
            <div className="p-8">
                <h2 className="mb-8 text-xl font-medium">Barangay ordinance</h2>
...
export default encodeBarangayOrdinance;
