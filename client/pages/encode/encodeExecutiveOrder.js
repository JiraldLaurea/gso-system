...
function encodeExecutiveOrder() {
    ...
    const submitDocument = async () => {
        if (
            ...
            await Axios.post(
                "http://localhost:3001/executiveOrder/getExecutiveOrderYear",
                data
            ).then(async (res) => {
                if (!res.data) {
                    setLoading(true);
                    ...
                    if (extension == "doc" || extension == "docx") {
                        const documentName = `ExecutiveOrder${response.selectedBarangay}${response.selectedDistrict}${yearSubmitted}.pdf`;
                        ...
                        const documentName = `ExecutiveOrder${response.selectedBarangay}${response.selectedDistrict}${yearSubmitted}.${extension}`;
    ...
    return (
        <div className="flex flex-col w-full max-w-5xl">
            <div className="p-8">
                <h2 className="mb-8 text-xl font-medium">Executive order</h2>
...
export default encodeExecutiveOrder;
