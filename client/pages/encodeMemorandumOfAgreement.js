...
function encodeMemorandumOfAgreement() {
    ...
    const submitDocument = async () => {
        if (file && dropdownMenuValueBarangay != "Barangay") {
            ...
            await Axios.post("http://localhost:3001/moa/getMoaYear", data).then(
                async (res) => {
                    if (!res.data) {
                        setLoading(true);

                        if (extension == "doc" || extension == "docx") {
                            const documentName = `Moa${response.selectedBarangay}${response.selectedDistrict}${yearSubmitted}.pdf`;
                            ...
                            await Axios.post(
                                ...
                                const moaUrl = await getDownloadURL(
                                    covertToPDFRef
                                );
                            ...
                            const documentName = `Moa${response.selectedBarangay}${response.selectedDistrict}${yearSubmitted}.${extension}`;
                            ...
                            const memorandumOfAgreementUrl =
...
export default encodeMemorandumOfAgreement;
