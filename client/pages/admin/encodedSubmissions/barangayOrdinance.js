...
function barangayOrdinance() {
    ...
    const { data: barangaysEncode } = useSWR(
        "http://localhost:3001/barangayOrdinance/getAllBarangayOrdinance"
    );
    ...
    const view = async (e) => {
        ...
        await Axios.post(
            "http://localhost:3001/barangayOrdinance/getBarangayOrdinance",
            data
        ).then((res) => {
            setImageUrl(res.data.barangayOrdinanceUrl);
        });
    };
