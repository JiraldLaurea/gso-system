...
function encodeFundingReq() {
    ...
    const submitDocument = async () => {
        if (file && dropdownMenuValueBarangay != "Barangay") {
            ...
            await Axios.post(
                "http://localhost:3001/fundingReq/getFundingReqYear",
                data
            ).then(async (res) => {
                if (!res.data) {
                    setLoading(true);
                    ...
                    if (extension == "doc" || extension == "docx") {
                        const documentName = `FundingReq${response.selectedBarangay}${response.selectedDistrict}${yearSubmitted}.pdf`;
                        ...
                        await Axios.post(
                            ...
                            const fundingReqUrl = await getDownloadURL(
                                covertToPDFRef
                            );
                        ...
                        const documentName = `FundingReq${response.selectedBarangay}${response.selectedDistrict}${yearSubmitted}.${extension}`;
...
export default encodeFundingReq;
